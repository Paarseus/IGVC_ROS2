...
- **wheel_separation_multiplier:** 1.19 — Mandow skid-steer correction (effective separation = 0.877 m). Same convention as `ros2_controllers/diff_drive_controller`; Husky uses 1.875, Jackal 1.5. Measured α=0.84 (open-loop ω delivery without correction was 82-86%; with multiplier the chassis delivers 100-103% on a smooth surface). **Re-calibrate for IGVC grass before competition** — α may change with surface friction. **Applied asymmetrically (forward kinematics only, NOT inverse `/wheel_odom`)** — see [`docs/skid_steer_kinematics_findings_2026_05_18.md`](docs/skid_steer_kinematics_findings_2026_05_18.md) for the empirical decomposition (chassis-side α≈0.96 + SparkMAX motor-delivery loss ≈85%) and EKF validation results (1×1m closed-loop square closes within 6 cm WITH multiplier, 63 cm WITHOUT).
...
- **Two configs, picked by `$ROS_DISTRO` in `navigation.launch.py:43-50`:**
  - **Humble (production, what runs on the Jetson):** `nav2_params_humble.yaml` + `navigate_igvc_autonav_humble.xml` BT (recovery BT — clear-costmap/wait/back-up/crawl + 45 s watchdog; **default since 2026-05-21**, was `navigate_to_pose_simple_humble.xml` which aborted on the first planner failure with no recovery). Point-to-point direct, no route graph. See [`docs/lidar_obstacle_avoidance_test_2026_05_21.md`](docs/lidar_obstacle_avoidance_test_2026_05_21.md)
...
| `ekf.yaml` | dual robot_localization EKF — **both EKFs fuse `/imu/data` + `/wheel_odom` (vx,vyaw)**; odom EKF additionally fuses ZED yaw-differential; map EKF additionally fuses `/odometry/gps` (differential, 4σ Mahalanobis gate). IMU has 5σ pose+twist rejection on both EKFs (Xsens stuck-bias gate, 2026-05-26). |
...
| **MPPI misses 20 Hz / "Optimizer fail to compute path" / costmap obstacle flicker during nav** | RViz2 running on the Jetson over NoMachine (software OpenGL) eats ~133% CPU; with gnome-shell + update-manager, load hit 10.75/8 cores → control loop starved → STVL consumed LiDAR in bursts → obstacles flickered (~40% lethal-cell sawtooth while stationary) → optimizer failed → abort. **Killing RViz dropped load 10.75→3.5 and cleared it.** Fix: never run RViz on the Jetson during nav — use **laptop Foxglove** (foxglove_bridge is in `navigation.launch.py`). Kill stray RViz with `pkill -x rviz2` (NOT `-f rviz2` — self-matches the shell, drops SSH). See [`docs/lidar_obstacle_avoidance_test_2026_05_21.md`](docs/lidar_obstacle_avoidance_test_2026_05_21.md). |
| Nav2 goal aborts on the first planner hiccup / robot won't replan around obstacles | Old default BT `navigate_to_pose_simple_humble.xml` has **no recovery behaviors** — one `ComputePathToPose` failure (e.g. a transient TF extrapolation error) collapses the PipelineSequence and aborts the goal. Fix: use `navigate_igvc_autonav_humble.xml` (recovery BT — clear-costmap/wait/back-up/crawl + 3× retry + 45 s watchdog). **Now the Humble default** (2026-05-21). |
| Planner "Extrapolation Error … [odom]→[map]" / "Could not transform the start or goal pose" | Sending the NavigateToPose goal in the **odom frame** forces an `odom→map` transform every cycle using the goal's fixed-at-send-time stamp, which ages past the `map→odom` buffer. Fix: send goals in the **map frame** (`map→odom` is stable, ±0.007°). For a relative goal, snapshot the current map pose and send a fixed map-frame target. |
...
| Xsens MTi-680G stuck with gyro bias ~70× normal (~-2.86°/s stationary drift, was -0.04°/s earlier) | The Xsens internal sensor-fusion filter can get stuck with a bad gyro bias estimate (likely from thermal drift + vibration during heavy use disqualifying its automatic Zero-Rotation-Update). ROS-level driver restart does NOT fix — filter state lives in IMU firmware. **Fix: physical USB power-cycle of the Xsens module.** Device gets a new tty (e.g. ttyUSB0→ttyUSB1); `xsens.yaml` has `scan_for_devices: true` so the driver finds it automatically after `sensors.launch.py` relaunches. **Symptom to watch for: chassis doesn't drive straight + `/imu/data.orientation` yaw drifts visibly while stationary.** The heading-hold cannot compensate for a bad IMU input — it ends up steering the chassis off-course. See [`docs/skid_steer_kinematics_findings_2026_05_18.md`](docs/skid_steer_kinematics_findings_2026_05_18.md) §6 for the full incident write-up. **2026-05-26: both EKFs now run with `imu0_pose_rejection_threshold: 5.0` + `imu0_twist_rejection_threshold: 5.0` (Mahalanobis σ gates), so the bad gyro values are now rejected at the filter level instead of flowing into the state estimate — the chassis no longer veers silently on a stuck Xsens. USB power-cycle is still the recovery step, but the filter buys time to notice.** |
...
| ZED VIO trajectory direction is rotated vs `base_link` (T3 finding #4) | The wrapper hardcodes `odomMsg->child_frame_id = "<cam>_camera_link"` (`zed_camera_component_main.cpp:1593,6354`) — not YAML-configurable. `robot_localization`'s differential-pose path applies only a world→world rotation (`ros_filter.cpp:~3013`) and does **not** apply a lever-arm (ω×r) correction, so fusing ZED's x/y from a non-base-mounted camera injects ω·lever phantom lateral velocity on every turn (~0.34 m/s at 0.5 rad/s with our 0.68 m forward mount). **Fix:** fuse yaw only — drop `x,y` from `odom1_config` in `ekf.yaml`. Yaw rate is invariant under rigid translation, so the lever-arm bug doesn't apply to angular fusion. Full derivation: [`docs/zed_vio_frame_analysis.md`](docs/zed_vio_frame_analysis.md). |

---

## TODOs

See [`TODO.md`](TODO.md) at the repo root — single source of truth, organized by priority.
