...
- **Actuator-node slew caps (softened 2026-05-19 for 50-60 lb chassis momentum):** max_linear_accel = 0.3 m/s², max_linear_decel = 0.8 m/s², max_angular_accel = 0.6 rad/s². At max_linear_mps=1.5 these give 5 s ramp-up, 1.9 s stop (1.4 m stopping distance), 2.5 s angular ramp. Decel stays > accel for e-stop safety. All `[DYNAMIC]` — re-tune via `ros2 param set` if Nav2 MPPI tracking degrades (the 2.8 s rollout horizon expects chassis to reach commanded speeds).
