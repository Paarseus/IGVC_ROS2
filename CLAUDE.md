...
| `align_depth.enable: true` in realsense.yaml silently ignored | realsense-ros 4.56.4 does not honor dot-separated nested params from `--params-file`. `/camera/camera/aligned_depth_to_color/image_raw` topic exists but publishes at 0 Hz, breaking any downstream `(color, aligned_depth)` sync. **Fixed in `sensors.launch.py`** by passing `{'align_depth.enable': True}` as an inline parameter dict alongside `realsense_config`. |
| `lane_obstacle_layer` never marks cells (layer-level z filter default) | nav2_costmap_2d::ObstacleLayer applies BOTH layer-level and per-source `min/max_obstacle_height` filters (intersection). Layer-level defaults to `[0.0, 2.0]`, silently overriding per-source even when set. Ground-plane lane points land at z < 0 with the current base_link mount geometry → rejected by default. **Fixed in `nav2_params*.yaml`** by adding explicit `min_obstacle_height: -0.5` and `max_obstacle_height: 0.3` at the `lane_obstacle_layer` level, not just under `lane_points`. TODO: tighten after `base_footprint → base_link` TF is calibrated so ground is reliably at z=0. |
| VLP-16 concentric ground-ring artifact in costmap | Each of the 16 laser elevations hits the ground at a specific distance (`h/tan(angle)`) — 6-8 discrete lethal rings at those radii. Passes the default voxel_layer z-filter when the sensor is low. Fixed by raising `velodyne_points.min_obstacle_height` to `-0.2` in both voxel_layer and obstacle_layer. Structural fix via patchwork++ ground segmentation pending. |
| Local costmap too coarse for lane perception | At 0.2 m/cell, 5cm lane paint becomes a 20cm virtual wall and `lane_inflation_layer.inflation_radius: 0.10` is smaller than one cell → effectively no-op. Fixed by setting local costmap `width: 20, height: 20, resolution: 0.05`. Global stays at 0.2 to keep SmacPlannerHybrid planning time reasonable. |
| `costmap_test.launch.py` loaded wrong nav2 params on Humble | Hardcoded `nav2_params.yaml` (Jazzy file). On Humble this silently loaded schema that omitted `always_send_full_costmap: true`, so `/local_costmap/costmap` only sent a one-shot transient_local snapshot and never republished. Fixed with a `ROS_DISTRO` check matching the pattern in `navigation.launch.py`. |
